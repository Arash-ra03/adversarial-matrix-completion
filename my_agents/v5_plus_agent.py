"""Matrix Arena competition agent.

solve()
-------
Models the observed entries as a sum of four complementary signals:
  (1) additive row/column bias   (median-polish sweeps)
  (2) inductive bilinear term    X @ W @ Z.T   (ridge on Kronecker features)
  (3) latent low-rank term       P @ Q.T       (vectorised ALS on the
  ...
  (4) weak nonlinear correction  sum_l a_l tanh(x_i.p_l * z_j.q_l)  (ridge on
      random tanh features, fit on the remaining residual)

Hyperparameters (ridge alpha, ALS rank/lambda, nonlinear alpha) are chosen by
a held-out grid search, then the four signals are combined via an
out-of-fold (K-fold) ridge stacking regression so the blend weights are
estimated honestly (no leakage), and finally every component is refit on ALL
observed entries at the selected hyperparameters before applying the learned
blend weights - so no observed data is wasted in the final answer. Wall-clock
is checked throughout against a conservative fraction of solve_timeout_s so
the function always returns a valid array well inside the hard deadline.
...
direction and split into a handful of blocks; each block gets an internally
...
harder is the *conductance* of the observation graph, and this construction
minimises it while remaining technically connected. Falls back to an honest
random-regular mask whenever anything is atypical, so it can never return an
invalid mask.
"""
...
class Agent:
    ...
    def solve(self, X, Z, Y_obs, mask, budget, seed):
        ...
        # v5 was conservative at 55% of the timeout.  The public runs showed
        # no timeouts, so use a little more budget for the richer stack while
        # still leaving a sizeable guard band for the grader wrapper.
        deadline = t0 + 0.68 * timeout
        ...
        names = ["mean", "bias", "direct_lr", "bilinear", "joint", "full"]
        ...
        for kf in range(K):
            ...
            # Pure low-rank is the piece that v2/v4 exploit well on the
            # low-rank-dominant seeds.  Keep it as its own OOF expert instead
            # of forcing every latent signal to be a residual of bilinear.
            if time.perf_counter() < deadline:
                try:
                    Pd, Qd = _als_fit(
                        fr_r, fr_c, fr_y - mean_f, n, m,
                        best_rank, max(best_lam, 0.2), max(24, n_iter_fold // 2), rng
                    )
                    oof["direct_lr"][va_mask_f] = mean_f + np.sum(Pd[fv_r] * Qd[fv_c], axis=1)
                except Exception:
        ...
        if time.perf_counter() < deadline:
            try:
                Pd_all, Qd_all = _als_fit(
                    ...
                    best_rank, max(best_lam, 0.2), max(40, n_iter_final // 2), rng
                )
